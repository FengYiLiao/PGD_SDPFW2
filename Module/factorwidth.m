function [Anew, bnew, cnew, Knew, info] = factorwidth(A,b,c,K,opts,idx)
...
% -------------------------------------------------------------------------    
    ...
    Anew   = [];
    [hei,wei] = size(A);
    dx = sqrt(wei)/opts.nop;
    NumOfsmallvars = (2*dx)^2;
    NumOfvars =  (2*dx)^2*opts.nop;
    Anew = zeros(height(A),NumOfvars);
    cnew   = [];
    cnew = zeros(NumOfvars,1);
...
%             Ak{k}  = Apsd(:,Index);
%             ck{k}  = cpsd(Index,:);
            Anew(:,start:start+NumOfsmallvars-1) = Apsd(:,Index); 
            cnew(start:start+NumOfsmallvars-1) = cpsd(Index,:);
            start = start + NumOfsmallvars;
            %Anew = [Anew,Ak{k}];
            %cnew = [cnew;ck{k}];
          %%%  %Ech  = [Ech;Index + Count]; 
        end 
